...
public class Aerolinea
{
    ...
    public Vuelo getVuelo( String codigoRuta, String fechaVuelo )
    {
        for(Vuelo vuelo : vuelos) {
        	...
        	String codigoRutaV = vuelo.getRuta().getCodigoRuta();
        	
        	if(fecha == fechaVuelo && codigoRutaV == codigoRuta) {
        		return vuelo;
        	}
        }
        return null;
    ...
    public Collection<Tiquete> getTiquetes( )
    {
        ...
        for(Vuelo vuelo : vuelos) { 
        	Collection<Tiquete> CollectionTiquetes = vuelo.getTiquetes();
        	for(Tiquete tiquete : CollectionTiquetes ) {
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	Avion AvionPropuesto = null;
    	for(Avion avion : aviones) {
    		if(avion.getNombre() == "nombreAvion") {    			
    			AvionPropuesto = avion;
    		}
    	}
    	if(AvionPropuesto == null) {
    		throw new Exception("No hay avion con ese nombre");
    	}
    	...
    	for(Vuelo vuelo : vuelos) {
    		try {
    			...
    			if(AvionVuelo.getNombre() == nombreAvion && FechaVuelo == "fecha") {
    				throw new Exception("El avion ya esta siendo usado en otro vuelo");
    			}
    		} catch (Exception e) {
		...
		Vuelo nuevo_vuelo = new Vuelo(fecha, ruta, AvionPropuesto);
	    	...
	    	for(Vuelo vuelo : vuelos) {
	    		if(vuelo.getRuta().getCodigoRuta() == codigoRuta) {
	    	    	...
	    	    	if(mes == "06" || mes == "07" || mes == "08" || mes == "12") {
	    	    		vuelo.venderTiquetes(cliente, CalculadoraTarifasTemporadaAlta, cantidad);
	    	    		break;	    	    		
	    	    	}else {
	    	    		vuelo.venderTiquetes(cliente, CalculadoraTarifasTemporadaBaja, cantidad);
	    	    		break;
	    	    	}
	    		}
	    	}
	    	
    	}catch(VueloSobrevendidoException ve){
    ...
    public void registrarVueloRealizado( String fecha, String codigoRuta )
    {
		...
		while(iterador.hasNext()) {
			Vuelo elemento = iterador.next();
			String codigoRutaVuelo = elemento.getRuta().getCodigoRuta();
    		String fechaVuelo = elemento.getFecha();
			if(codigoRutaVuelo == codigoRuta && fechaVuelo == fecha) {
				vuelos.add(elemento);
				iterador.remove();
			}
		}
